...
```bash
feroxbuster -u http://$IP -w /usr/share/seclists/Discovery/Web-Content/raft-medium-directories.txt -x php,html,txt --filter-status 404
```

Recursive enumeration with depth limit:

```bash
feroxbuster -u http://$IP -w /usr/share/seclists/Discovery/Web-Content/common.txt -d 3
```

Silent mode (pipe-friendly output):

```bash
feroxbuster -u http://$IP -w /usr/share/seclists/Discovery/Web-Content/common.txt -q --silent
```
...
```bash
ffuf -u http://$IP/FUZZ -w /usr/share/seclists/Discovery/Web-Content/admin-panels.txt -fc 404
```

Find backup files:

```bash
ffuf -u http://$IP/FUZZ -w /usr/share/seclists/Discovery/Web-Content/backup-files.txt -fc 404
```
...
```bash
gobuster dir -u http://$IP -w /usr/share/wordlists/dirb/common.txt -x php,html,txt -b 404 -q
```

Recursive directory enumeration:

```bash
gobuster dir -u http://$IP -w /usr/share/wordlists/dirb/common.txt -r
```
